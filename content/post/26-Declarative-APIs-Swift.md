...
*Note: Some types names might seem a little odd, but they mostly follow an app-specific ontology (e.g. Operation). Others should simply be renamed; still waiting for that refactor capabilities for Swift code...* 
...
I think this is just a first step into a future where we'll see sophisticated libraries hide their complexity by providing simple, declarative, interfaces. And hopefully, some day, we'll get a declarative UI library for building iOS apps[^2].
